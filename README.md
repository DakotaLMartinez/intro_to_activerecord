...
|Update|`@book.update(sold: true)`|a hash of attributes where the keys are the columns to be updated and the corresponding values the desired new column values.|The updated `@book` is returned or `false` if any validations failed when trying to update `@book`|Instance method|
